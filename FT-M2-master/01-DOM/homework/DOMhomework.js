...
function buildToDo(todo, index) {
  ...
  let toDoText = document.createElement('span'); // <span id=3 class='completeText' >Terminar de leer el libro</span>
  ...
  // <div class='toDoShell'>
  //  <span id=3 class='completeText' >Terminar de leer el libro</span>
  //</div> 
...
function buildToDos(toDos) {
  // Tu código acá:
  let resultBuildToDo = toDos.map(function(todo, index){ return buildToDo(todo, index) });
    return resultBuildToDo;
}
...
function displayToDos() {
  // Tu código acá:
  let toDoContainer = document.querySelector('#toDoContainer');//<div id="toDoContainer"></div>
  toDoContainer.innerHTML = "";

  let resultBuildToDos = buildToDos(toDoItems);
  //[<div class='toDoShell'>
  //  <span id=0 >Aprobar el M2</span>
  //</div>,
  // <div class='toDoShell'>
  //  <span id=1 >Comprar papas</span>
  //</div>,
  // <div class='toDoShell'>
  //  <span id=2 >Ir al gimanasio</span>
  //</div>,
  // <div class='toDoShell'>
  //  <span id=3 class='completeText' >Terminar de leer el libro</span>
  //</div> 
  ...
  for(let i = 0; i < resultBuildToDos.length; i++) {
    toDoContainer.appendChild(resultBuildToDos[i]);
...
function addToDo() {
  ...
  if(input.value !== '') {
    let newToDo = new ToDo(input.value);// {description: 'Terminar de leer el libro', complete: false}
    toDoItems.push(newToDo);
    // [{description: 'Aprobar el M2', complete: false}
    // {description: 'Comprar papas', complete: false},
    // {description: 'Ir al gimanasio', complete: false},
    // {description: 'Terminar de leer el libro', complete: false}]
    input.value = '';
    displayToDos();
  }
}
